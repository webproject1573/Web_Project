...
spis2 = []
print('Введите токен')
tok = input()   # просим ввести токен, узнать его можно, зайдя в диалог с BotFather
...
for link in html.find_all('a'):   # прасим по тегам a -> href
    a = link.get('href')
    if 'doc' in a:    # здесь мы собираем все ссылки на полную статью, они являются продолжением одной большой ссылки, поэтому начинаются на doc 
    	spis.append(a)
