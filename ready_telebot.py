...
'сражаются с различными противниками.': 'https://yadi.sk/d/yDw1Pv3-Co4b7',
 ...
 ' знаменитых персонажей комиксов Marvel.': 'https://yadi.sk/d/oNs_D_VR65v1E',
  ...
  ' которых никак нельзя назвать похожими друг на друга': 'https://yadi.sk/d/K1TeMV1y65WLW'}
   ...
   # создадим список названий этих игр, чтобы в дальнейшем на рандом выбрать какую нибудь игру
...
def get_text_messages(message):
    ...
    if 'привет' in message.text.lower():
    ...
    elif 'rpg' in message.text.lower() or 'рпг' in message.text.lower():
        f = random.choice(spis_rpg)   # на рандом выбираем игру и ищем ее в словаре
    ...
    elif 'adventure' in message.text.lower() or 'приключения' in message.text.lower():
        g = random.choice(spis_adv)
        for key, val in adv.items():
            if g in key:
                bot.send_message(message.from_user.id, "Нашел классную игрушку! - {}, а скачать ее для psp можно вот тут - {}".format(key, val))

    elif 'arcada' in message.text.lower() or 'аркада' in message.text.lower():
        ...
        bot.send_message(message.from_user.id, "Я тебя не понимаю. Напиши Привет.")   # сценарий, если бот не понимает команду
